module BankSimulator
  ...
  class DepositFundsService
    ...
    XML_REQUEST_FILE = Rails.root.join("app/services/bank_simulator/deposit.xml")
    ...
    def exchange_for_token
      begin
        request = DepositRequest.post("/bankSimulator.php?transaction=deposit", body: payload)
      ...
      if request.response.code == "200"
        ...
        if code != "1000"
          @errors << "There's been an error making the deposit [#{code}: #{status}]"
        else
          @output = reference
        end
      else
    ...
    def payload
      ...
      transaction_hash["deposit"]["amount"] = @transaction_params[:amount]
      transaction_hash["deposit"]["cardToken"] = @transaction_params[:bank_token]

      transaction_hash["deposit"].to_xml(root: "deposit").to_s
    end
  end
